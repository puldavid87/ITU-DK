...
void aes (uint8_t *text_in, uint8_t *text_out){
  //10 roundkey  
 for(m=0;m<10;m++){
  if(m==0){  //first round
    for(i=0;i<limit;i++){ //text and ciper arrays to matrix  [4][4]
     for(j=0;j<limit;j++){
        ciper[j][i]=EEPROM.read(k);
 ...
 void updated_key(uint8_t *text_in){
    for(i=0;i<16;i++){ 
       EEPROM.update(i,text_in[i]);
       delay(10);
      }
  }

  void change_key (uint8_t *text_in,uint8_t gate){
      for(i=0;i<16;i++){ 
         EEPROM.update(i,text_in[i]^gate);
        delay(10);
      }
  }
